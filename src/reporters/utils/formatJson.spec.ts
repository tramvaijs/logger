...
describe('@tinkoff/logger/reporters/utils/formatJson', () => {
  it('should format errors', () => {
    ...
    expect(
      formatError(
        Object.assign(new Error(message), { stack, prop, body, code })
      )
    ).toEqual({
      message,
      stack,
  ...
  it('should format to json view', () => {
    ...
    expect(
      formatJson({ name: 'test', args: ['trace 123'], date, level: 0 })
    ).toEqual({
      name: 'test',
      message: 'trace 123',
      date,
    ...
    expect(
      formatJson({
        ...
        level: 5,
        args: [
          Object.assign(new Error('arg2'), { stack: 'mockStack' }),
          'custom message',
        ],
      })
    ).toEqual({
    ...
    expect(
      formatJson({
        ...
        level: 6,
        args: [
          Object.assign(new Error('arg3'), { stack: 'mockStack' }),
          [1, 2],
          { a: 3 },
        ],
      })
    ...
    expect(
      formatJson({
        ...
        args: [
          '1',
          2,
          Object.assign(new Error('arg4'), { stack: 'mockStack' }),
        ],
      })
    ).toEqual({
      name: 'test',
